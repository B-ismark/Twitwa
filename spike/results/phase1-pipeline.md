...
| **totals** | **526 checks, 102 mutations, every one observed failing.** Counts taken from a run of all nine suites and both gates, not written from memory — they had gone stale in this table four times now (446/84, then 510/93). 96 of the mutations are in the suites; the other 6 are in the two gates that check their own rules. **A clone runs 516 and reports 10 skipped**, which is the honest figure: 7 need an unpublished capture, 3 need a generated `android/` |
