...
true: **1666 of the 1684 checks run in a clone**, and the 18 that cannot say so
...
none (**1684 checks and 270 mutations** counting the two PNG tools, all four
...
**A clone runs 1666 of those 1684 and reports 18 skipped**, and it runs all 270
mutations with none surviving. Measured 2026-09-23 from `git archive` of
the commit that added Back handling, extracted with CRLF line endings as a Windows clone gets it, so the thing gated is the commit and not the
...
That pairing is the check: 1684 − 1666 = 18, which is the skip count, so the
...
cd spike && node src/shell.test.mjs      # 90 checks on the editor's tool sessions, Style controls and Back
cd spike && node src/autocrop.test.mjs   # 84 checks on the crop the editor opens on
cd spike && node src/update.test.mjs     # 128 checks on the update check, its URL allowlist, and its contract with the Kotlin
...
cd spike && node tools/check-dead.mjs    # 163 exports + 7 self-checks on its own rule
...
done                                     # silence is the pass; 270 mutations
```
